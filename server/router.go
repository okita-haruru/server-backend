...
func NewRouter(server *Server, conf config.Config, socketserver *socketio.Server) *gin.Engine {
	...
	r.GET("/ranking/fish_amount", server.controller.HandleGetFishRankingByAmount)
	r.GET("/ranking/fish_size", server.controller.HandleGetFishRankingBySize)
	r.GET("/ranking/total_amount", server.controller.HandleGetFishRankingByTotalAmount)
